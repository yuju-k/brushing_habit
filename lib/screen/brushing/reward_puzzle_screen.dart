...
class _RewardPuzzleState extends State<RewardPuzzle> {
  ...
  Widget build(BuildContext context) {
    late String readPuzzleFileName; //퍼즐조각 파일이름
    late String readPuzzleFolder; //퍼즐조각 폴더이름
    late String
        readPuzzleOriginal; //퍼즐조각 원본파일 DB문서이름(collection:readPuzzleOriginal)
    //late int readPuzzlePosition; //퍼즐조각이 맞춰지는 위치 1~6
    ...
    Future getPuzzle() async {
      ...
      var doc =
          await firestore.collection('puzzle_slice').doc(puzzleName).get();
      readPuzzleFileName = doc['file_name']; //파일이름
      readPuzzleFolder = doc['folder']; //저장되어있는 폴더
      readPuzzleOriginal =
          doc['original']; //원본파일 DB문서이름 (collection:readPuzzleOriginal)
      //readPuzzlePosition = doc['position']; //파일조각이 맞춰지는 위치 1~6

      //collection: getItems / uid / puzzleOriginal에 readPuzzleOriginal 추가
      //puzzle_original이 이미 있으면 추가하지 않음
      var doc3 = await firestore.collection('getItems').doc(uid).get();
      if (doc3.exists) {
        //uid가 있으면
        if ((doc3.data() as Map<String, dynamic>)
            .containsKey('puzzleOriginal')) {
          var puzzleOriginal = doc3['puzzleOriginal'];
          if (puzzleOriginal.contains(readPuzzleOriginal)) {
            //print('이미 등록되어있는 Origianl정보입니다..');
          } else {
            FirebaseFirestore.instance.collection('getItems').doc(uid).set({
              'puzzleOriginal': FieldValue.arrayUnion([readPuzzleOriginal])
            }, SetOptions(merge: true));
          }
        } else {
          FirebaseFirestore.instance.collection('getItems').doc(uid).set({
            ...
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return Column(
                ...
                children: [
                  ...
                  Image.asset(
                    'assets/puzzles/$readPuzzleFolder/$readPuzzleFileName',
                    width: 130,
                    height: 130,
                  ),
                  const SizedBox(
